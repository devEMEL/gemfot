...
contract FeeEscrow is Ownable {
    ...
    function allocateFees(
        ...
        (address launch,,, uint tokenId) = indexer.poolIndex(_poolId);
        if (tokenId != 0 && IERC721(launch).ownerOf(tokenId) == _recipient) {
            totalFeesAllocated[_poolId] += _amount;
        }

        // Transfer USDC from the sender into this escrow
