 ...
 *
 * @dev Copied from the `v4-core` `PoolSwapTest.sol` contract and simplified to suit Launch
 * requirements.
 */
contract PoolSwap is IUnlockCallback {
     ...
     * @return delta_ The BalanceDelta of the swap
     */
    function swap(PoolKey memory _key, SwapParams memory _params, bytes memory _hookData) public payable virtual returns (BalanceDelta delta_) {
        delta_ = abi.decode(
