...
import { Token } from '../../src/generated/schema'
...
describe('Price calculations', () => {
  test('calculateAmountUSD calculates correctly', () => {
    ...
    const token0DerivedUSDC = BigDecimal.fromString('0.1')
    const token1DerivedUSDC = BigDecimal.fromString('0.2')
    const ethPriceUSD = BigDecimal.fromString('1000')

    const result = calculateAmountUSD(amount0, amount1, token0DerivedUSDC, token1DerivedUSDC, ethPriceUSD)
