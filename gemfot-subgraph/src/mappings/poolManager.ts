...
import { Initialize as InitializeEvent } from '../../generated/PoolManager/PoolManager'
import { Pool, PoolManager, Token } from '../../generated/schema'
import { getSubgraphConfig, SubgraphConfig } from '../utils/chains'
import { ADDRESS_ZERO, ZERO_BD, ZERO_BI, ONE_BI } from '../utils/constants'
import { updatePoolDayData, updatePoolHourData } from '../utils/intervalUpdates'
import { findNativePerToken, sqrtPriceX96ToTokenPrices } from '../utils/pricing'
import { fetchTokenDecimals, fetchTokenName, fetchTokenSymbol, fetchTokenTotalSupply } from '../utils/token'
...
export function handleInitializeHelper(
  ...
  log.debug('INITIALIZE_HANDLER_ENTERED', [])
  const poolManagerAddress = subgraphConfig.poolManagerAddress
  const whitelistTokens = subgraphConfig.whitelistTokens
  const tokenOverrides = subgraphConfig.tokenOverrides
  const poolsToSkip = subgraphConfig.poolsToSkip
  ...
  if (poolsToSkip.includes(poolId)) return

  // ─── 1. PoolManager (create if first pool) ───
  let poolManager = PoolManager.load(poolManagerAddress)
  if (poolManager === null) {
    poolManager = new PoolManager(poolManagerAddress)
    poolManager.poolCount = ZERO_BI
  ...
  // ─── 2. Tokens (create if first time seeing them) ───
  let token0 = Token.load(event.params.currency0.toHexString())
  if (token0 === null) {
    token0 = new Token(event.params.currency0.toHexString())
    token0.symbol = fetchTokenSymbol(event.params.currency0, tokenOverrides, nativeTokenDetails)
    ...
    const decimals = fetchTokenDecimals(event.params.currency0, tokenOverrides, nativeTokenDetails)
    if (decimals === null) {
      log.debug('handleInitialize: token0 decimals null, skipping pool {}', [poolId])
      return
    }
  ...
  if (token1 === null) {
    ...
    const decimals = fetchTokenDecimals(event.params.currency1, tokenOverrides, nativeTokenDetails)
    if (decimals === null) {
      log.debug('handleInitialize: token1 decimals null, skipping pool {}', [poolId])
      return
    }
  ...
  // ─── 3. Whitelist pools for price derivation ───
  const pool = new Pool(poolId)
  ...
  // ─── 4. Derive prices (must happen after whitelist pools are set) ───
  token0.derivedUSDC = findNativePerToken(token0, wrappedNativeAddress, stablecoinAddresses)
  token1.derivedUSDC = findNativePerToken(token1, wrappedNativeAddress, stablecoinAddresses)
  ...
  // ─── 6. Persist everything before snapshots ───
  token0.save()
  token1.save()
  pool.save()
  poolManager.save()

  // ─── 7. Snapshots (see correct derivedUSDC and pool prices) ───
  updatePoolDayData(poolId, event)
  updatePoolHourData(poolId, event)
}
